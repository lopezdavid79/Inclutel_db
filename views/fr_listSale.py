...
class AgregarVentaDialog(wx.Dialog):
    def __init__(self, parent,gestion_ventas):
        ...
        self.btn_seleccionar_cliente = wx.Button(panel, label="Seleccionar &    Cliente")
        vbox.Add(self.btn_seleccionar_cliente, flag=wx.ALIGN_RIGHT | wx.RIGHT | wx.TOP, border=10)

        # Eventos
        self.btn_filtrar_cliente.Bind(wx.EVT_BUTTON, self.filtrar_clientes)
        self.btn_seleccionar_cliente.Bind(wx.EVT_BUTTON, self.seleccionar_cliente)

        self.txt_cliente.Bind(wx.EVT_KEY_DOWN, self.on_key_cliente)
        self.lista_clientes.Bind(wx.EVT_KEY_DOWN, self.on_key_lista_enter)
